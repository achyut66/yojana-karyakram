...
class Napilagatanumanbreak
{
	protected static $table_name = "napi_lagat_anuman_break";
	protected static $db_fields = array('id','break_work_name','task_count','length','breadth'
                                            ,'height','total_evaluation','deduct_part','plan_id','sno_taken','period','break_no');
	public $id;
	public $break_work_name;
        public $task_count;
        public $length;
        public $breadth;
        public $height;
	...
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$id} order by sno_taken");
		return $result_array;
	}
        public static function find_by_plan_id_period($plan_id=0,$period=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and period={$period} order by sno_taken asc");
		return $result_array;
	}
        public static function find_by_plan_id_sno($plan_id=0,$sno_taken=0,$period=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and sno_taken={$sno_taken} and period={$period}");
		return $result_array;
	}
	...
	public static function find_by_id($id=0)
	{
        ...
        public static function find_single_row($plan_id=0,$sno_taken=0,$period=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and sno_taken={$sno_taken} and period={$period} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
...
//                print_r($clean_attributes);exit;
    ...
    $starting_fund=new Planstartingfund();
?>