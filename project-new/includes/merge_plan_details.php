...
class Mergeplandetails
{
        protected static $table_name = "merge_plan_id";
	protected static $db_fields = array('id','plan_id','parent_plan_ids','merged_amount');
    	public $id;
    	public $plan_id;
        public $parent_plan_ids;
        public $merged_amount;
	...
	public function delete()
	{
		...
		public function find_by_parent_id()
	{
		global $database;
		// delete from table where condition limit 1
		$sql = "SELECT t1.*,t2.* from child_plan_details t1 inner join plan_details1 t2 on t1.plan_id = t2.id" ;
$object_array = array();
		while ($row = $database->fetch_array($result_set))
		{
			$object_array[]= self::instantiate($row);
