...
class Analysisbasedwithdraw
{
	protected static $table_name = "analysis_based_withdraw";
	protected static $db_fields = array('id','payment_evaluation_count','get_qty','get_unit_id','evaluated_date','evaluated_date_english','evaluated_amount','payable_amount','advance_payment','contengency_amount','renovate_amount','dpr_amount','due_amount',
            'disaster_management_amount','total_amount_deducted','total_paid_amount','plan_id','created_date','created_date_english');
	public $id;
	...
	public $get_unit_id;
        ...
        // Common database method
         public static function sum_katti_by_name($string,$plan_id=0)
	{
		global $database;
		$sql = "select sum($string) from ".self::$table_name." where plan_id={$plan_id}";
//                echo $sql;exit;
		...
		$total= array_shift($row);
          ...
          public static function getevaluated_Amount($plan_id=0)
	{
		global $database;
		$sql = "select sum(evaluated_amount) from ".self::$table_name." where plan_id={$plan_id}";
//                echo $sql;exit;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		$total= array_shift($row);
	...
	{
		global $database;
		$sql = "select sum(renovate_amount) + sum(dpr_amount) + sum(disaster_management_amount) + sum(due_amount) + sum(advance_payment) from ".self::$table_name." where plan_id={$plan_id}";
//                echo $sql;exit;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		$total= array_shift($row);
          ...
          public static function getTotalContngencyAmount($plan_id=0)
	{
		global $database;
		$sql = "select sum(contengency_amount) from ".self::$table_name." where plan_id={$plan_id}";
//                echo $sql;exit;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		$total= array_shift($row);
        ...
        public static function getTotalDharautiAmount($plan_id=0)
	{
		global $database;
...
//                echo $sql;exit;
	...
	{
		global $database;
		$sql = "select sum(payable_amount) from ".self::$table_name." where plan_id={$plan_id}";
//                echo $sql;exit;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		$total= array_shift($row);
