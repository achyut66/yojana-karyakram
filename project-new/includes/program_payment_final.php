...
class Programpaymentfinal
{
    protected static $table_name = "program_payment_final";
    protected static $db_fields = array('id','sn','program_remaining_amount',
        'paid_date','paid_date_english','total_payment_amount','pan_total_amt',
        'pan_tds','vat_total_amt','vat_tds','vat_amount','total_bill','tax_tot',
        'more_less','payment_taken_amount','congentency_amount','maintainance_amount',
        'deposit_amount','emergency_amount','total_amount','net_total_amount','program_id',
        'enlist_id','katti_amt','marmat','cont','bipat');
    public $id;
    public $sn;
    public $bipat;
    public $cont;
    public $marmat;
    public $program_remaining_amount;
    public $paid_date;
    public $paid_date_english;
    public $total_payment_amount;
    public $pan_total_amt;
    public $pan_tds;
    public $vat_total_amt;
    public $vat_tds;
    public $vat_amount;
    public $total_bill;
    public $tax_tot;
    public $more_less;
    public $payment_taken_amount;
    public $congentency_amount;
    public $maintainance_amount;
    public $deposit_amount;
    public $emergency_amount;
    public $total_amount;
    public $net_total_amount;
    public $program_id;
    public $enlist_id;
    public $katti_amt;
    ...
    {
        global $database;
        $result_array = self::find_by_sql("select * from ".self::$table_name."  where id in(".$plan_id_string.") order by paid_date_english ASC");
        return $result_array;
    }
    ...
    public static function find_by_program_id_and_sn_asc($program_id=0,$sn=0)
    {
        global $database;
        $result_array=self::find_by_sql("select * from ".self::$table_name. " where program_id={$program_id} and sn={$sn}");
        return !empty($result_array)? array_shift($result_array) : false;
    }
    ...
    public static function get_total_payment_amount_sum($program_id)
    {
        global $database;
        $sql = "select sum(total_payment_amount) from ".self::$table_name." where program_id=".$program_id;
        $result_set = $database->query($sql);
        $row = $database->fetch_array($result_set);
        return array_shift($row);
    }
    public static function get_net_total_amount_sum($program_id)
    {
        global $database;
        $sql = "select sum(net_total_amount) from ".self::$table_name." where program_id=".$program_id;
        $result_set = $database->query($sql);
        $row = $database->fetch_array($result_set);
        return array_shift($row);
    }
    public static function get_net_total_amount_sum_for_all_programs()
    {
        global $database;
        $sql = "select sum(net_total_amount) from ".self::$table_name;
        $result_set = $database->query($sql);
        $row = $database->fetch_array($result_set);
        return array_shift($row);
    }
