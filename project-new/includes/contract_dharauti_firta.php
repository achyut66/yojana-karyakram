...
class Contractdharautifirta
{
	protected static $table_name = "contract_dharuti_firta";
	protected static $db_fields = array('id','payment_evaluation_count','contractor_id','dharauti_return_amount','taken_date','taken_date_english','plan_id');
        public $id;
        public $payment_evaluation_count;
        public $contractor_id;
        public $dharauti_return_amount;
        public $taken_date;
        public $taken_date_english;
        public $plan_id;
        
        // Common database method
         public static function find_by_contractor_id_plan_id($contractor_id,$plan_id)
	{
		global $database;
                $sql="select * from ".self::$table_name. " where contractor_id=$contractor_id and plan_id=".$plan_id;
         	$result_array=self::find_by_sql($sql);
		return $result_array;
	}
        public static function find_by_max_plan_and_contractor($num,$plan_id,$contractor_id)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where payment_evaluation_count='".$num."' and plan_id=$plan_id and contractor_id=$contractor_id limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
        public static function getTotalPayableAmountByContractorId($plan_id=0,$contractor_id)
	{
		global $database;
		$sql = "select sum(dharauti_return_amount) from ".self::$table_name." where plan_id={$plan_id} and contractor_id=".$contractor_id;
//                echo $sql;exit;
        ...
        public static function getTotalPayableAmount($plan_id=0)
	{
		global $database;
		$sql = "select sum(dharauti_return_amount) from ".self::$table_name." where plan_id={$plan_id}";
//                echo $sql;exit;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		$total= array_shift($row);
	...
	public static function find_by_id($id=0)
	{
        ...
        public static function find_by_payment_count_of_contractor($count=1,$plan_id=0,$contractor_id)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and payment_evaluation_count={$count} and contractor_id=$contractor_id limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
	public static function find_by_payment_count($count=1,$plan_id=0)
	{
        ...
        public static function find_by_plan_ids($id=0)
	{
		global $database;
	...
	public static function count_all()
	{
        ...
        public static function  getMaxInsallmentByPlanIdAndContractorId($plan_id=0,$contractor_id)
	{
		global $database;
		$sql = "select max(payment_evaluation_count) from ".self::$table_name." where plan_id={$plan_id} and contractor_id=".$contractor_id;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
...
$contingency= new Contingencyexenditure();
