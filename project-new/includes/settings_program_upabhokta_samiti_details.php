...
class Upabhoktasamitidetails
{
	protected static $table_name = "settings_program_upabhokta_samiti_details";
	protected static $db_fields = array('id','post_id','name','address','gender','cit_no','issued_district','mobile_no','tol_id','created_date','created_date_english');
        public $id;
        public $post_id ;
        public $name ;
        public $address ;
        ...
        public static function find_by_post_plan_id($id,$tol_id)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where post_id=$id  and tol_id={$tol_id} limit 1");
	...
	public static function find_by_plan_id_post_id($plan_id=0,$post_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and post_id={$post_id} limit 1 ");
		return !empty($result_array)? array_shift($result_array) : false;
	}
    ...
    public static function find_by_tol_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where tol_id={$id} ");
		return $result_array;
	}
	public static function find_adakshya($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where tol_id={$id} and post_id=1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
