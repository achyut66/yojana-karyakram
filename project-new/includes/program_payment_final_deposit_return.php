...
class Programpaymentdepositreturn
{
	protected static $table_name = "program_payment_final_deposit_return";
	protected static $db_fields = array('id','period','program_id','sn','deposit_amount','enlist_id','date');
	public $id;
        public $period;
	public $program_id;
        public $sn;
          ...
          public static function maxPeriodForProgram($program_id=0,$sn=0)
	{
		...
		return array_shift($row);
            ...
            public static function getsum($program_id=0,$sn=0)
	{
		global $database;
		$sql = "select sum(deposit_amount) from ".self::$table_name." where program_id={$program_id} and sn={$sn}";
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
	    ...
	    public static function find_by_program_id_and_sn($program_id=0,$sn=0)
	{
		...
		return !empty($result_array)? array_shift($result_array) : false;
            ...
            public static function find_by_program_id_sn_and_period($program_id=0,$sn=0,$period=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where program_id={$program_id} and sn={$sn} and period={$period} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
