...
class Estimatelagatanumanbreak
{
	protected static $table_name = "estimate_lagat_anuman_break";
	protected static $db_fields = array('id','break_work_name','task_count','length','breadth'
                                            ,'height','total_evaluation','deduct_part','plan_id','sno_taken','break_no');
	public $id;
	...
	public static function find_by_plan_id($id=0)
	{
        ...
        public static function find_by_plan_id_sno($plan_id=0,$sno_taken=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and sno_taken={$sno_taken}");
		return $result_array;
	}
	public static function find_by_id($id=0)
	{
        ...
        public static function find_single_row($plan_id=0,$sno_taken=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where plan_id={$plan_id} and sno_taken={$sno_taken} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
