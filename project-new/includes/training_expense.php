...
{	
        protected static $table_name = "training_expense";
	protected static $db_fields = array('id','description','unit','rate','quantity','total','remarks','plan_id','sn');
	public $id;
	public $description ;
        public $unit;
        public $rate;
	...
	{
		global $database;
		return self::find_by_sql("select * from ".self::$table_name." where plan_id= {$plan_id}");
		 	
	}
          public static function find_by_program_id_and_sn($plan_id,$sn)
	{
		global $database;
		return self::find_by_sql("select * from ".self::$table_name." where plan_id= {$plan_id} and sn={$sn}");
		 	
	}
        public static function find_grand_total_by_plan_id($plan_id,$sn)
	{
		global $database;
		$sql = "select sum(total) from ".self::$table_name." where plan_id = {$plan_id} and sn={$sn}";
        ...
        public function getName($string_id="")
        {
                 ...
                 return $topic_names = $topic_selected->work_name;   
        }
