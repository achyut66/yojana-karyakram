...
class Programtimeadditionaffiliation
{
	protected static $table_name = "program_time_addition_affiliation";
	protected static $db_fields = array('id','sn','period','program_problem_reason','letter_date','letter_date_english',
            'decesion_date','decesion_date_english','extended_date','extended_date_english','program_id');
	public $id;
        public $sn;
	public $period ;
        public $program_problem_reason ;
        public $letter_date;
        public $letter_date_english;
        public $decesion_date;
        public $decesion_date_english;
        public $extended_date;
        public $extended_date_english;
         public $program_id;
	...
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where period='".$num."' and plan_id=$plan_id limit 1");
	      ...
	      public static function find_by_program_id_and_sn($program_id=0,$sn=0)
	{
	 ...
	 public static function find_by_program_id($plan_id)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where program_id={$plan_id} ");
		return $result_array;
	}
        public static function find_by_program_period($period=0,$plan_id=1)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where program_id={$plan_id} and period={$period} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
	...
	public static function count_all()
	{
        ...
        public static function maxPeriodForProgram($program_id=0,$sn=0)
	{
		global $database;
		$sql = "select max(period) from ".self::$table_name." where program_id={$program_id} and sn={$sn}";
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
