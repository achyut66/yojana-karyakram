...
class Contractmoredetails
{
	protected static $table_name = "contract_more_details";
	protected static $db_fields = array('id','budget','work_order_date','work_order_budget','start_date','start_date_english','completion_date','completion_date_english','venue','samjhauta_party','post_id_3','miti','total_family_members','male','female','total_members','plan_id');
	public $id;
        public $budget;
...
// Common database method
	...
	{
		global $database;
		$sql = "select count(*) from ".self::$table_name." where program_id=".$program_id;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
