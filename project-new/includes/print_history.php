...
class PrintHistory
{
    protected static $table_name = "print_history";
    protected static $db_fields = array('id', 'url', 'plan_id', 'user_id', 'nepali_date', 'english_date', 'worker1', 'worker2', 'worker3', 'worker4','type_name1','type_name2','type_name3','type_name4');
    public $id;
    public $url;
    public $plan_id;
    public $user_id;
    ...
    public function setEmptyObject()
    {
        $self = new self;
        return $self;
    }
    ...
    {
        global $database;
        $result_array = self::find_by_sql("select * from " . self::$table_name . " where url='" . $url . "' and plan_id = {$plan_id} order by english_date");
        return !empty($result_array) ? array_shift($result_array) : false;
    }

    public static function find_by_url_plan_id_and_letter_type($url = "", $plan_id = 0,$letter_type='')
    {
        global $database;
        $result_array = self::find_by_sql("select * from " . self::$table_name . " where url='" . $url . "' and plan_id = {$plan_id} order by english_date and letter_type='".$letter_type."'");
        return !empty($result_array) ? array_shift($result_array) : false;
    }
