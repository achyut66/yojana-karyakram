...
class Engdate
{
	protected static $table_name = "eng_dates";
	protected static $db_fields = array('id','date_english','date_nepali','type','period','plan_id');
        protected static $date_types = array('estimate'=>1,'napi'=>2,'bill'=>3,'completion'=>4);
	public $id;
	public $date_english;
	public $date_nepali;
	public $type;
	...
	public static function find_by_id($id=0)
	{
        ...
        public static function get_estimate_date($plan_id=0)
        {
            $sql = "select * from ".self::$table_name." where plan_id={$plan_id} and type=".self::$date_types['estimate']." limit 1";
        ...
        public static function get_bill_date($plan_id=0,$period=0)
        {
            $sql = "select * from ".self::$table_name." where plan_id={$plan_id} and period={$period} and type=".self::$date_types['bill']." limit 1";
            $result_array = self::find_by_sql($sql);
            if(!empty($result_array))
            {
                return array_shift($result_array);
        ...
        public static function get_completion_date($plan_id=0)
        {
            $sql = "select * from ".self::$table_name." where plan_id={$plan_id} and type=".self::$date_types['completion']." limit 1";
            $result_array = self::find_by_sql($sql);
            if(!empty($result_array))
            {
                return array_shift($result_array);
