...
class Units
{
    ...
    protected static $db_fields = array('id','name', 'alias');
    public $id;
    public $name;
    public $alias;
    // Common database method
    public static function get_id_by_name($id)
    {
        global $database;
        $result_array=self::find_by_sql("select * from ".self::$table_name. " where name='".$id."' limit 1");
        $result = array_shift($result_array);
        return $result->id;
    }

    public static function get_id_by_name_or_alias($id)
    {
        global $database;
        $result_array=self::find_by_sql("select * from ".self::$table_name. " where name='".$id."' or find_in_set('".$id."',alias) limit 1");
        // $result = array_shift($result_array);
        return !empty($result_array)? array_shift($result_array)->id : -1;
        // return $result->id;
    }
