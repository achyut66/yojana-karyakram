...
{
	protected static $table_name = "program_details";
	protected static $db_fields = array('id','fiscal_id','topic_area_id','topic_area_type_id','topic_area_type_sub_id','topic_area_agreement_id','topic_area_investment_id','program_name','ward_no','investment_amount');
	public $id;
	public $fiscal_id;
            ...
            $total_pages=$pagination->total_pages();
             if($pagination->total_count>$per_page){
                if($pagination->has_previous_page()){//check if it has previous page function used from class
                    $prev_link='<a href="'.$link.'?page_no='.$pagination->previous_page().'">आघिल्लो</a>';
                 }
                    else{
                        $prev_link="";
                    }
                    //check if it has next page function used from class
                        ...
                        $next_link='<a href="'.$link.'?page_no='.$pagination->next_page().'">पछिल्लो</a>';
                        }
                    else{
                        $next_link="";
                    }
                ...
                for($i=1;$i<=$total_pages;$i++){
                    if($i==$pagination->current_page){
                        $html.="<span style='color:red; background:black; padding-top:1px; padding-right:5px; padding-buttom:1px; padding-left:5px;'>".convertedcit($pagination->current_page)."</span>";
                    }
                    else{
                        $html.='<span> <a href="'.$link.'?page_no='.$i.'">'.convertedcit($i).'</span>';
