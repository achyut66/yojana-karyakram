...
class LetterFormat
{
    protected static $table_name = "letter_format";
    protected static $db_fields = array('id', 'plan_type', 'letter_type', 'letter_text', 'date');
    public $id;
    public $plan_type;
    public $letter_type;
    public $letter_text;
    ...
    public static function find_by_plan_type($plan_type)
    {
        global $database;
        $result_array = self::find_by_sql("select * from " . self::$table_name . " where plan_type='" . $plan_type . "'");
        return !empty($result_array) ? ($result_array) : false;
    }

    public static function find_by_plan_type_and_letter_type($plan_type, $letter_type)
    {
        global $database;
        $result_array = self::find_by_sql("select * from " . self::$table_name . " where plan_type='" . $plan_type . "' and letter_type='" . $letter_type . "' limit 1");
    ...
    public static function find_by_ind($id = 0)
    {
        global $database;
        $result_array = self::find_by_sql("select * from " . self::$table_name . " where id={$id} limit 1");
        return !empty($result_array) ? array_shift($result_array) : false;
